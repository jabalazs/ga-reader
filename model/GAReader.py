...
class Model:
    def __init__(
        ...
        doc_var, query_var, cand_var = (
            T.itensor3("doc"),
            T.itensor3("quer"),
            T.wtensor3("cand"),
        )
        docmask_var, qmask_var, candmask_var = (
            T.bmatrix("doc_mask"),
            T.bmatrix("q_mask"),
            T.bmatrix("c_mask"),
        )
        ...
        self.predicted_probs, predicted_probs_val, self.network, W_emb, attentions = self.build_network(
            K, vocab_size, W_init
        )

        self.loss_fn = T.nnet.categorical_crossentropy(
            self.predicted_probs, target_var
        ).mean()
        self.eval_fn = lasagne.objectives.categorical_accuracy(
            self.predicted_probs, target_var
        ).mean()

        loss_fn_val = T.nnet.categorical_crossentropy(
            predicted_probs_val, target_var
        ).mean()
        ...
        updates = lasagne.updates.adam(
            self.loss_fn, self.params, learning_rate=self.learning_rate
        )
    ...
    def build_network(self, K, vocab_size, W_init):
        ...
        l_tokin = L.InputLayer(
            shape=(None, config.MAX_WORD_LEN), input_var=self.inps[8]
        )
        l_tokmask = L.InputLayer(
            shape=(None, config.MAX_WORD_LEN), input_var=self.inps[9]
        )
        ...
        l_docembed = L.EmbeddingLayer(
            l_docin, input_size=vocab_size, output_size=self.embed_dim, W=W_init
        )  # B x N x 1 x DE
        l_doce = L.ReshapeLayer(
            l_docembed, (doc_shp[0], doc_shp[1], self.embed_dim)
        )  # B x N x DE
        l_qemb = L.EmbeddingLayer(
            l_qin,
        ...
        l_qembed = L.ReshapeLayer(
            l_qemb, (qry_shp[0], qry_shp[1], self.embed_dim)
        )  # B x N x DE
        l_fembed = L.EmbeddingLayer(
            l_featin, input_size=2, output_size=2
        )  # B x N x 2
        ...
        # char embeddings
        if self.use_chars:
            l_lookup = L.EmbeddingLayer(
                l_tokin, self.num_chars, self.char_dim
            )  # T x L x D
            l_fgru = L.GRULayer(
                l_lookup,
                self.char_dim,
                ...
                gradient_steps=config.GRAD_STEPS,
            ...
            l_fwdembed = L.DenseLayer(
                l_fgru, self.embed_dim / 2, nonlinearity=None
            )  # T x DE/2
            l_bckembed = L.DenseLayer(
                l_bgru, self.embed_dim / 2, nonlinearity=None
            )  # T x DE/2
        ...
        l_prob = AttentionSumLayer(
            [l_doc, l_q], self.inps[4], self.inps[12], mask_input=self.inps[10]
        )
