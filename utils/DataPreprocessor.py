...
class DataPreprocessor:
    ...
    def make_dictionary(self, question_dir, vocab_file):
        ...
        print(
            "%d other tokens (including @placeholder, %s and %s)"
    ...
    def parse_one_file(self, fname, dictionary, use_chars):
        ...
        cand_raw = map(
            lambda x: x.strip().split(":")[0].split(), raw[8:]
        )  # candidate answers
        ...
        try:
            ...
            at = qry_raw.index("@")
            qry_raw = (
                qry_raw[:at] + ["".join(qry_raw[at : at + 2])] + qry_raw[at + 2 :]
            )
            cloze = qry_raw.index("@placeholder")
        ...
        if use_chars:
            doc_chars = map(
                lambda w: map(
                    lambda c: c_dict.get(c, c_dict[" "]), list(w)[:MAX_WORD_LEN]
                ),
                doc_raw,
            )
            qry_chars = map(
    ...
    def parse_all_files(self, directory, dictionary, use_chars):
        ...
        questions = [
            self.parse_one_file(f, dictionary, use_chars) + (f,)
            for f in all_files
        ]
        return questions
