...
class MiniBatchLoader:
    ...
    def reset(self):
        ...
        for l, ixs in self.bins.items():
            ...
            ixs_list = [
                (ixs[self.batch_size * i : min(n, self.batch_size * (i + 1))], l)
                for i in range(k)
            ]
    ...
    def next(self):
        ...
        dw = np.zeros(
            (curr_batch_size, curr_max_doc_len, 1), dtype="int32"
        )  # document words
        qw = np.zeros(
            (curr_batch_size, self.max_qry_len, 1), dtype="int32"
        )  # query words
        c = np.zeros(
            (curr_batch_size, curr_max_doc_len, self.max_num_cand), dtype="int16"
        )  # candidate answers
        cl = np.zeros(
            (curr_batch_size,), dtype="int32"
        ...
        m_qw = np.zeros(
            (curr_batch_size, self.max_qry_len), dtype="int32"
        )  # query word mask
        m_c = np.zeros(
            (curr_batch_size, curr_max_doc_len), dtype="int32"
        )  # candidate mask
        ...
        for n, ix in enumerate(ixs):
            ...
            doc_w, qry_w, ans, cand, doc_c, qry_c, cloze, fname = self.questions[
                ix
            ]
        ...
        tt = np.zeros(
            (len(types), self.max_word_len), dtype="int32"
        )  # type characters
